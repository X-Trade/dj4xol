...
class TestDiplomacyView(TestCase):
    ...
    def test_diplomacy_fleet_names_use_locate_links_in_contract_list(self):
        game = default_game(stars=5, fleets=1)
        player = game.players.first()
        race_type = get_default_race_type()
        other_user = User.objects.create_user('diplo_fleet_link', 'diplo_fleet_link@test.com', 'pass')
        other_account = Account.objects.create(django_user=other_user, alias='DFL')
        other_player = Player.objects.create(
            game=game,
            account=other_account,
            name='Fleet Link Race',
            plural_name='Fleet Link Races',
            race_type=race_type,
        )
