...
class AdministrationAutomationTest(TestCase):
    ...
    def test_stale_scout_mission_state_is_cleared_when_orders_finish(self):
        ...
        state = MicromanagerObjectState.objects.create(
            game=self.game,
            player=self.player,
            target_type=MicromanagerObjectState.TARGET_FLEET,
            target_fleet=fleet,
        ...
        state.set_tags(['scout'])
        state.set_data({'route_star_ids': []})
        state.save(update_fields=['tags_json', 'data_json'])

        active_ids = GameTurn(self.game)._active_auto_exploration_fleet_ids(
            self.player
        )

        self.assertNotIn(fleet.id, active_ids)
        self.assertFalse(
            MicromanagerObjectState.objects.filter(id=state.id).exists()
        )
    ...
    def test_tier_four_mechanical_high_employment_prioritises_growth_over_mines(self):
        ...
        self.assertTrue(planned)
        self.assertEqual(planned[0], 'BUILD_COLONISTS_1K')
        self.assertNotIn('BUILD_MINE', planned)
    ...
    def test_ai_micromanager_level_five_skips_fleet_build_when_dispatchable_idle_fleet_exists(self):
        self.player.is_ai = True
        self.player.ai_module = 'micromanager'
        self.player.save(update_fields=['is_ai', 'ai_module'])
        self.star.has_administration = False
    ...
    def test_ai_micromanager_level_five_can_queue_fleet_build_with_reserved_single_idle_fleet(self):
        self.player.is_ai = True
        self.player.ai_module = 'micromanager'
        self.player.save(update_fields=['is_ai', 'ai_module'])
        self.star.has_administration = False
    ...
    def test_ai_micromanager_level_five_deletes_unstarted_auto_fleet_build_at_orbit_cap(self):
        self.player.is_ai = True
        self.player.ai_module = 'micromanager'
        self.player.save(update_fields=['is_ai', 'ai_module'])
        self.star.has_administration = False
    ...
    def test_ai_micromanager_small_homeworld_can_spare_colonists_for_colonise(self):
        self.player.is_ai = True
        self.player.ai_module = 'micromanager'
        self.player.save(update_fields=['is_ai', 'ai_module'])
        self.star.has_administration = False
    ...
    def test_auto_colonise_uses_mission_state_to_skip_reserved_target(self):
        self.player.is_ai = True
        self.player.ai_module = 'expansionist'
        self.player.save(update_fields=['is_ai', 'ai_module'])
        self.star.colonists = 220_000
        ...
        targets = list(self.game.stars.exclude(id=self.star.id).order_by('id')[:2])
        reserved, fallback = targets
        for offset, target in enumerate((reserved, fallback), start=1):
            target.player = None
            target.x = int(self.star.x) + offset
            target.y = int(self.star.y)
            target.gravity = self.player.gravity_center
        ...
        existing_fleet = Fleet.objects.create(
            game=self.game,
            player=self.player,
            name='State Reserved Expedition',
            x=self.star.x,
            y=self.star.y,
            ship_count=1,
            cargo_capacity=120,
        ...
        state = MicromanagerObjectState.objects.create(
            ...
            player=self.player,
            target_type=MicromanagerObjectState.TARGET_FLEET,
            target_fleet=existing_fleet,
            mission='colonise',
            updated_year=self.game.year,
        )
        state.set_data({'target_star_id': str(reserved.id)})
        state.save(update_fields=['data_json'])
        dispatch = Fleet.objects.create(
            game=self.game,
            player=self.player,
            name='Fresh Colony Ship',
    ...
    def test_expansionist_builds_fleet_when_scout_mission_pool_is_thin(self):
        self._create_administration_tech(4, 4)
        self.player.is_ai = True
        self.player.ai_module = 'expansionist'
        self.player.save(update_fields=['is_ai', 'ai_module'])
        ...
        self.star.germanium_inventory = 100_000
        self.star.ironium_yield = 80
        self.star.boranium_yield = 80
        self.star.germanium_yield = 80
        self.star.save()
        self.star.production_orders.all().delete()
        self.player.fleets.all().delete()
        ...
        orbit_fleets = []
        for idx in range(4):
            orbit_fleets.append(Fleet.objects.create(
                game=self.game,
                player=self.player,
    ...
    def test_ai_micromanager_level_five_can_reassign_patrol_fleet_to_colonise(self):
        self.player.is_ai = True
        self.player.ai_module = 'micromanager'
        self.player.save(update_fields=['is_ai', 'ai_module'])
        self.star.has_administration = False
        ...
        for fleet in (guard, scout):
            if fleet.orders.filter(order_type='COLONISE').exists():
                colonise_fleets.append(fleet)
        self.assertEqual(len(colonise_fleets), 1)
        reassigned = colonise_fleets[0]
        self.assertFalse(reassigned.orders.filter(order_type='PATROL').exists())

    def test_ai_micromanager_patrol_rebalances_to_stronger_available_fleet(self):
        self.player.is_ai = True
        self.player.ai_module = 'micromanager'
        self.player.save(update_fields=['is_ai', 'ai_module'])
        self.star.has_administration = False
        ...
        weak_patrol = Fleet.objects.create(
            game=self.game,
            player=self.player,
            name='Weak Patrol',
            x=self.star.x,
        ...
        strong_idle = Fleet.objects.create(
            game=self.game,
            player=self.player,
            name='Strong Idle',
            x=self.star.x,
        ...
        self.assertFalse(weak_patrol.orders.filter(order_type='PATROL').exists())
        self.assertTrue(
            strong_idle.orders.filter(
                order_type='PATROL',
                repeat=True,
                added_by_micromanager=True,
            ).exists()
    ...
    def test_ai_micromanager_patrol_keeps_only_top_slice_of_available_fleets(self):
        self.player.is_ai = True
        self.player.ai_module = 'micromanager'
        self.player.save(update_fields=['is_ai', 'ai_module'])
        self.star.has_administration = False
    ...
    def test_ai_micromanager_patrol_leaves_one_idle_fleet_when_possible(self):
        self.player.is_ai = True
        self.player.ai_module = 'micromanager'
        self.player.save(update_fields=['is_ai', 'ai_module'])
        self.star.has_administration = False
    ...
    def test_micromanager_skips_reported_targets_that_are_now_colonised(self):
        self.player.is_ai = True
        self.player.ai_module = 'micromanager'
        self.player.save(update_fields=['is_ai', 'ai_module'])

        occupied = self.game.stars.exclude(id=self.star.id).order_by('id').first()
        vacant = self.game.stars.exclude(id__in=[self.star.id, occupied.id]).order_by('id').first()

        occupied.player = self.player
        occupied.colonists = 10_000
        occupied.x = int(self.star.x) + 1
        occupied.y = int(self.star.y)
        occupied.gravity = float(self.player.gravity_center)
        occupied.temperature = float(self.player.temperature_center)
        occupied.radiation = float(self.player.radiation_center)
        occupied.save(update_fields=[
            'player',
            'colonists',
            'x',
            'y',
        ...
        self._create_star_report(occupied, is_survivable=True)

        vacant.player = None
        vacant.colonists = 0
        vacant.x = int(self.star.x) + 2
        vacant.y = int(self.star.y)
        vacant.gravity = float(self.player.gravity_center)
        vacant.temperature = float(self.player.temperature_center)
        vacant.radiation = float(self.player.radiation_center)
        vacant.save(update_fields=[
            'player',
            'colonists',
            'x',
            'y',
        ...
        self._create_star_report(vacant, is_survivable=True)

        best = GameTurn(self.game)._best_colonise_target_for_colony(self.star)
        self.assertIsNotNone(best)
        self.assertEqual(best.id, vacant.id)
